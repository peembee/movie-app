...
export const APIShowContext = createContext();
...
export function APIShowProvider({ children }) {
  const [showData, setShowData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  const fetchShowTesting = async () => {
    try {
  ...
  // }, []);
  return (
    <APIShowContext.Provider value={{ showData, fetchShowTesting }}>
      <div>
        {isLoading ? <h2>Loading shows...</h2> : <h2>shows is now loaded</h2>}
      </div>
      {children}
    </APIShowContext.Provider>
  );
}
